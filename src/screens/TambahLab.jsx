...
export class TambahLab extends React.Component {
	...
	handleFormSubmit(e) {
		...
		Appointment.tambahLabPasien(dataJson).then(response => {
            console.log(response.status)
			...
			if (response.status === 200) {
				...
				alert(`Sukses menambahkan lab dengan nama ${this.state.pasien.nama}`)
			} else {
				this.setState({
					loading : false
				})
				alert(`Gagal menambahkan lab dengan nama ${this.state.pasien.nama}`)
			}
		})
	}
